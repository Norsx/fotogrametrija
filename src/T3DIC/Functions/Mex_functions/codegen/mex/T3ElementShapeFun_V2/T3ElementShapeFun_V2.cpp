                  ...
                  const coder::array<boolean_T, 1U> &in2)
{
  ...
  b_in1.set_size(&gb_emlrtRTEI, &sp, loop_ub);
  stride_0_0 = (in1.size(0) != 1);
  stride_1_0 = (in2.size(0) != 1);
  for (int32_T i{0}; i < loop_ub; i++) {
    b_in1[i] = (in1[i * stride_0_0] && in2[i * stride_1_0]);
  }
  in1.set_size(&gb_emlrtRTEI, &sp, loop_ub);
  for (int32_T i{0}; i < loop_ub; i++) {
    in1[i] = b_in1[i];
  }
                             ...
                             const coder::array<real_T, 1U> &in3)
{
  ...
  in1.set_size(&p_emlrtRTEI, &sp, loop_ub);
                          ...
                          coder::array<real_T, 2U> &vec)
{
  ...
  emlrtHeapReferenceStackEnterFcnR2012b((emlrtConstCTX)sp);
  loop_ub = X.size(0);
  N.set_size(&h_emlrtRTEI, sp, X.size(0), 3);
  nz = X.size(0) * 3;
  for (r1 = 0; r1 < nz; r1++) {
    N[r1] = 0.0;
  }
  ...
  nz = Y.size(0);
  P_global.set_size(&i_emlrtRTEI, &b_st, Y.size(0), 2);
  for (r1 = 0; r1 < nz; r1++) {
    P_global[r1] = Y[r1];
  }
  ...
  if (P_global.size(0) == 0) {
    P_normalized.set_size(&k_emlrtRTEI, &b_st, 2, 0);
  } else {
  ...
  P_normalized.set_size(&m_emlrtRTEI, sp, 2, b_P_normalized.size(1));
  rtemp = b_P_normalized.size(1) << 1;
  for (r1 = 0; r1 < rtemp; r1++) {
    P_normalized[r1] = b_P_normalized[r1];
  }
  xi.set_size(&n_emlrtRTEI, sp, b_P_normalized.size(1));
  eta.set_size(&o_emlrtRTEI, sp, b_P_normalized.size(1));
  for (r1 = 0; r1 < nz; r1++) {
  ...
  if (xi.size(0) == eta.size(0)) {
    b_r2.set_size(&p_emlrtRTEI, sp, b_P_normalized.size(1));
    rtemp = (xi.size(0) / 2) << 1;
  ...
  emlrtSubAssignSizeCheckR2012b(((coder::array<real_T, 1U> *)&X)->size(), 1,
                                b_r2.size(), 1, &emlrtECI, (emlrtCTX)sp);
  for (r1 = 0; r1 < loop_ub; r1++) {
    N[r1] = b_r2[r1];
  }
  emlrtSubAssignSizeCheckR2012b(((coder::array<real_T, 1U> *)&X)->size(), 1,
                                xi.size(), 1, &b_emlrtECI, (emlrtCTX)sp);
  for (r1 = 0; r1 < loop_ub; r1++) {
    N[r1 + N.size(0)] = xi[r1];
  }
  emlrtSubAssignSizeCheckR2012b(((coder::array<real_T, 1U> *)&X)->size(), 1,
                                eta.size(), 1, &c_emlrtECI, (emlrtCTX)sp);
  for (r1 = 0; r1 < loop_ub; r1++) {
    N[r1 + N.size(0) * 2] = eta[r1];
  }
  Mask.set_size(&q_emlrtRTEI, sp, X.size(0));
  b_r3.set_size(&r_emlrtRTEI, sp, X.size(0));
  for (r1 = 0; r1 < loop_ub; r1++) {
    maxval = N[r1];
    Mask[r1] = (maxval >= 0.0);
    b_r3[r1] = (maxval <= 1.0);
  }
  r4.set_size(&s_emlrtRTEI, sp, X.size(0));
  r5.set_size(&t_emlrtRTEI, sp, X.size(0));
  for (r1 = 0; r1 < loop_ub; r1++) {
  ...
  b_r3.set_size(&u_emlrtRTEI, sp, X.size(0));
  r5.set_size(&v_emlrtRTEI, sp, X.size(0));
  for (r1 = 0; r1 < loop_ub; r1++) {
  ...
  if ((Mask.size(0) != X.size(0)) &&
    ...
    emlrtDimSizeImpxCheckR2021b(Mask.size(0), X.size(0), &d_emlrtECI,
                                (emlrtConstCTX)sp);
  }
  ...
  //  keyboard
  vec.set_size(&w_emlrtRTEI, sp, nz, 2);
  loop_ub = nz << 1;
  for (r1 = 0; r1 < loop_ub; r1++) {
    vec[r1] = 0.0;
  }
  ...
  r6.set_size(&x_emlrtRTEI, sp, rtemp);
  rtemp = 0;
  for (r1 = 0; r1 <= e_Transform_tmp; r1++) {
    if (Mask[r1]) {
      r6[rtemp] = r1;
  ...
  for (r1 = 0; r1 < e_Transform_tmp; r1++) {
    if (r6[r1] > X.size(0) - 1) {
      emlrtDynamicBoundsCheckR2012b(r6[r1], 0, X.size(0) - 1, &emlrtBCI,
                                    (emlrtConstCTX)sp);
    }
  }
  emlrtSubAssignSizeCheckR2012b(&nz, 1, r6.size(), 1, &e_emlrtECI,
                                (emlrtCTX)sp);
  for (r1 = 0; r1 < nz; r1++) {
    vec[r1] = X[r6[r1]];
  }
  for (r1 = 0; r1 < e_Transform_tmp; r1++) {
    if (r6[r1] > Y.size(0) - 1) {
      emlrtDynamicBoundsCheckR2012b(r6[r1], 0, Y.size(0) - 1, &b_emlrtBCI,
                                    (emlrtConstCTX)sp);
    }
  }
  rtemp = vec.size(0);
  emlrtSubAssignSizeCheckR2012b(vec.size(), 1, r6.size(), 1, &f_emlrtECI,
                                (emlrtCTX)sp);
  for (r1 = 0; r1 < rtemp; r1++) {
  ...
  //  keyboard
  st.site = &e_emlrtRSI;
  coder::b_sparse(st, static_cast<real_T>(nz), psi_x);
  st.site = &f_emlrtRSI;
  coder::b_sparse(st, static_cast<real_T>(nz), psi_y);
  b_N.set_size(&y_emlrtRTEI, sp, r6.size(0), 3);
  for (r1 = 0; r1 < 3; r1++) {
    for (rtemp = 0; rtemp < e_Transform_tmp; rtemp++) {
      if (r6[rtemp] > N.size(0) - 1) {
        emlrtDynamicBoundsCheckR2012b(r6[rtemp], 0, N.size(0) - 1, &c_emlrtBCI,
                                      (emlrtConstCTX)sp);
      }
  ...
  b_N.set_size(&ab_emlrtRTEI, sp, r6.size(0), 3);
  for (r1 = 0; r1 < 3; r1++) {
    for (rtemp = 0; rtemp < e_Transform_tmp; rtemp++) {
      if (r6[rtemp] > N.size(0) - 1) {
        emlrtDynamicBoundsCheckR2012b(r6[rtemp], 0, N.size(0) - 1, &d_emlrtBCI,
                                      (emlrtConstCTX)sp);
      }
      b_N[rtemp + b_N.size(0) * r1] = N[r6[rtemp] + N.size(0) * r1];
    }
